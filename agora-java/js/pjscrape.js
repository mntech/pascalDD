    ...
    var runner = (function() {
        ...
        ScraperSuite.prototype = {
            ...
            scrape: function(url, scrapers, complete) {
                ...
                if (url === 'http://www.labmanager.com/?go.jump/no/56b2') {
                	log.msg('Skipping known bad url: ' + url);
                    visited[url] = true;
					complete(false);
                    return;
				} else {
		    ...
		    // w/e, i dont need it 
// * CLIENT: TypeError: Result of expression 'page.resource' [undefined] is not an object. (undefined line 662)
                            ...
                            if (scrapers && scrapers.length) {
                                ...
                                scrapers.forEach(function(scraper) {
                                    if (isFunction(scraper)) {
                                        // standard scraper
										// CHANGE BY JBALINT to include depth
										var o = page.evaluate(scraper);
										if (o != null) {
											o.depth = suite.depth;
										}
										suite.addItem(o);
